	...
	<body>
		...
		<div class="page-container">
			<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
				<thead >
					<tr>
						<th colspan="7" scope="col">已申请拾物列表</th>
					</tr>
					<tr class="text-c">
						<th>拾物编号</th>
						<th>时间</th>
				...
				<tbody>
					<c:forEach items="${page.getRows()}" var="row">
						<tr class="text-c">
							<td >${row.pick_id}</td>
							<td >${row.pick_time}</td>
							<td >${row.pick_place}</td>
							<td >${row.pick_item_desc}</td>
							<td >${row.pick_phone}</td>
							<td >${row.pick_status}</td>
							<td class="f-14 td-manage">
								<a style="text-decoration:none" onClick="shenhe(this,${row.pick_id})" href="javascript:;" title="审核">审核</a> 
								<a style="text-decoration:none" class="ml-5" onClick="shanchu(this,${row.pick_id})" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
		...
		function shenhe(obj,id){
			...
			function(){
				$.post("${pageContext.request.contextPath }/shiwu/update.action",{"id":id},
					function(data){
						if(data =="OK"){
		               	 	alert("审核成功！");
		...
		function shanchu(obj,id){
			...
			function(){
				$.post("${pageContext.request.contextPath }/shiwu/delete.action",{"id":id},
						function(data){
							if(data =="OK"){
			               	 	alert("删除成功！");
			                	window.location.reload();
